...
\begin{definition}{Scheduling Policies}
    \begin{center}
    \begin{tabular}{l|l|c}
...
\begin{example2}{Exam: Thread Code Analysis (Task 4)}
    3 threads; each: lock mutex $\to$ \texttt{c = a++} (a starts 5) $\to$ set
    \texttt{SCHED\_FIFO} prio = c $\to$ \texttt{cond\_wait} $\to$ print
    \texttt{"Output c : b++"} $\to$ unlock. \texttt{main}: \texttt{cond\_broadcast}
    after 100 ms.
    \tcblower
    \begin{itemize}
        \item a) \important{3} threads started
        \item b) priorities \important{5, 6, 7} $\to$ prio \important{7} is highest
        \item c) \texttt{SCHED\_FIFO} = \important{preemptive} scheduling
        \item d) \texttt{cond\_broadcast} wakes \important{all} waiting threads
        \item e) \texttt{pthread\_join(tid[0])} waits only for the \emph{first} thread
              $\to$ fix: loop \texttt{pthread\_join(tid[i])} for all i
        \item f) output (highest prio first): \texttt{Output 7 : 0} /
              \texttt{Output 6 : 1} / \texttt{Output 5 : 2}
    \end{itemize}
\end{example2}

\begin{example2}{Exam: Scheduling Policy Comparison (Task 5)}
    \resizebox*{\linewidth}{!}{%
    \begin{tabular}{l|c|c|c}
         & \texttt{OTHER} & \texttt{RR} & \texttt{FIFO} \\\hline
        priority & nice $-20$..$+19$ & prio 1..99 & prio 1..99 \\
        highest & nice $-20$ & prio 99 & prio 99 \\
        realtime? & no & \important{yes} & \important{yes} \\
        preempt by & RT tasks + slices & higher RT + RR slice & higher RT task \\
