...
\begin{KR}{Build \& Install the Kernel}
    \textbf{Host:}
\begin{lstlisting}[language=bash, style=basesmol, aboveskip=2pt, belowskip=2pt]
make                       # build -> arch/<arch>/boot/{zImage,bzImage,Image}
make install               # as root: installs /boot/vmlinuz-<ver>,
                           # System.map-<ver>, config-<ver>; updates grub
make INSTALL_MOD_PATH=<dir>/ modules_install   # default /lib/modules/<ver>/
\end{lstlisting}
...
%% >>> IMAGE (slide 34): ARM ISR/exception vector table (Reset, NMI, HardFault, SVC #11 at 0x2C, SysTick, IRQ0..239) <<<
